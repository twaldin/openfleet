...
test('buildDashboardSurface renders a user-facing dashboard summary', () => {
  ...
  createTask(stateDir, { title: 'Polish ops UI', status: 'in_progress', assignee: 'coder' })

  const surface = buildDashboardSurface(stateDir)

  assert.match(surface, /OpenFleet dashboard @/)
  assert.match(surface, /Counts: agents=1 jobs=2 tasks=1/)
  ...
  assert.match(surface, /Polish ops UI \| status=in_progress \| assignee=coder \| instance=coder-gpt@macbook/)
